...
int main(int argc, char **argv) {
    ...
    // 3. GAMITIN ANG BAGONG STRUCT (Font11_Segoe)
    // Kusa na nitong kukunin ang size na 25x33 (o kung ano man ang kinalabasan)
    int gw = Font11_Segoe.Width;  
    int gh = Font11_Segoe.Height; 
    ...
    for(int i=0; i<cols; i++) {
        ...
        // Gamitin ang Font11 widths
        if (idx >= 0 && idx < 95) total_w += Font11_Segoe_Widths[idx];
    }
    ...
    for (int ci = 0; ci < cols; ++ci) {
        ...
        // 4. POINT SA TAMANG TABLE (Font11_Segoe_Table)
        const uint8_t *glyph = &Font11_Segoe_Table[idx * (gw * gh)];
        ...
        // Advance cursor
        cur_x += Font11_Segoe_Widths[idx];
    }

    // 5. SAVE WITH NEW NAME
    char filename[100];
    sprintf(filename, "pic/preview_font11_test.bmp");
